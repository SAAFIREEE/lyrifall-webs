...
function CursorLight() {
  ...
  return (
    <motion.div
      className="pointer-events-none fixed z-50 hidden h-96 w-96 -translate-x-1/2 -translate-y-1/2 rounded-full bg-purple-500/5 blur-[100px] md:block"
    ...
    <main className="relative overflow-x-hidden min-h-screen bg-[#050607]">
          ...
          <div className="grid w-full items-center gap-16 lg:grid-cols-[1.12fr_0.88fr]">
            ...
            <div className="relative z-10 text-center lg:text-left">
              ...
              <h1 className="gsap-soft text-balance max-w-4xl text-5xl font-extrabold leading-[1.05] tracking-tight text-white md:text-7xl">
                Next-generation synchronized <span className="bg-gradient-to-r from-[#7de3ff] via-purple-300 to-indigo-400 bg-clip-text text-transparent drop-shadow-[0_2px_15px_rgba(125,227,255,0.15)]">soundscapes.</span>
              </h1>
              ...
              <div className="gsap-soft mt-9 flex flex-wrap justify-center lg:justify-start gap-4">
                <a
                  ...
                  className="magnetic inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-7 py-3.5 text-sm font-semibold text-white shadow-xl shadow-cyan-500/5 transition-all"
                >
                  Invite to Discord <ArrowUpRight className="h-4 w-4" />
                </a>
                <a
                  href="#commands"
                  className="magnetic inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[0.03] px-7 py-3.5 text-sm font-medium text-neutral-300 transition-all"
                >
                  View App Commands
                </a>
              </div>
            ...
            <div className="relative z-10 flex justify-center items-center">
              ...
              >
                ...
                <div className="space-y-2.5 font-sans my-5 border-l-2 border-purple-500/30 pl-4 py-0.5 text-left bg-white/[0.01] p-3 rounded-r-xl border-y border-r border-white/[0.02]">
                  ...
                  <p className="text-xs text-neutral-300 flex items-center gap-2 font-light">
                    <span className="w-1 h-1 rounded-full bg-purple-400" /> Reaction tracking panels online via <code className="text-[11px] font-mono text-[#7de3ff]">/rolepanel</code>
                  </p>
                </div>
        ...
        <div className="section-shell">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {[
              ...
              <motion.article
                className="glass rounded-xl p-6 bg-[#0c0e12]/30"
                key={title}
          ...
          <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
            ...
            <div className="relative grid gap-4 sm:grid-cols-2">
              {skillClusters.map((cluster, index) => (
                <motion.article
                  className={`glass rounded-xl p-5 bg-[#0c0e12]/30 ${index === 2 ? "sm:col-span-2" : ""}`}
                  key={cluster.title}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true, margin: "-80px" }}
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {cluster.items.map((item) => (
                      <span
                        className="rounded-full border border-white/5 bg-white/[0.02] px-2.5 py-1 text-[10px] text-neutral-400 font-mono"
                        key={item}
                      >
                        {item}
                      </span>
      ...
      <section id="commands" className="relative scroll-mt-16">
        <div className="section-shell pb-32">
          ...
          <div className="glass rounded-xl overflow-hidden shadow-2xl bg-[#0c0e12]/20">
...
function TiltProject({ project, index }: { project: any; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="glass group rounded-xl p-4 [transform-style:preserve-3d] bg-[#0c0e12]/30"
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: "-80px" }}
      transition={{ duration: 0.6, delay: index * 0.05 }}
    >
      <div className={`relative h-44 overflow-hidden rounded-lg bg-gradient-to-br ${project.accent}`}>
        <div className="absolute inset-4 rounded-md border border-white/5 bg-[#050607]/80 backdrop-blur-xl" />
